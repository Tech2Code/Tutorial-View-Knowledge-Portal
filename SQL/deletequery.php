...
<tr><td class="bg"><a  class="tab1" target="_top"   href="deletequery.php">SQL - Delete Query</a><br/></td></tr>
...
<tr><td colspan="2"><marquee direction="left" scrolldellay="500" style="color:#996600;font-size:19;">SQL - DELETE Query</marquee>
</td></tr>
<tr><td colspan="2" align=center>
<img src="..\pic\student.jpg" alt=" My LOGO" align="middle" width="147" height="147"/></tr></td>

<tr>
<td style="text-align:left;"><a  href="updatequery.php"><img src="../pic/previous1.gif" width=90 height=30/></a></td>
<td style="text-align:right;"><a  href="likeclause.php"><img src="../pic/next1.gif" width=90 height=30/></a></td>
</tr>
<tr><td colspan="2">
<hr>
<p>The SQL <b>DELETE</b> Query is used to delete the existing records from a table.</p>
<p>You can use WHERE clause with DELETE query to delete selected rows, otherwise all the records would be deleted.</p>
<h2>Syntax:</h2>
<p>The basic syntax of DELETE query with WHERE clause is as follows:</p>
...
+----+----------+-----+-----------+----------+</pre>
<p>Following is an example, which would DELETE a customer, whose ID is 6:</p>
<pre class="output">SQL&gt; DELETE FROM CUSTOMERS
WHERE ID = 6;</pre>
<p>Now, CUSTOMERS table would have the following records:</p>
<pre class="output">+----+----------+-----+-----------+----------+
| ID | NAME     | AGE | ADDRESS   | SALARY   |
+----+----------+-----+-----------+----------+
|  1 | Ramesh   |  32 | Ahmedabad |  2000.00 |
...
|  7 | Muffy    |  24 | Indore    | 10000.00 |
+----+----------+-----+-----------+----------+</pre>
<p>If you want to DELETE all the records from CUSTOMERS table, you do not need to use WHERE clause and DELETE query would be as follows:</p>
<pre class="output">SQL&gt; DELETE FROM CUSTOMERS;</pre>
<p>Now, CUSTOMERS table would not have any record.</p>
