...
<tr><td class="bg"><a  class="tab1" target="_top"   href="topclause.php" >SQL - Top Clause</a><br/></td></tr>
...
<tr><td colspan="2"><marquee direction="left" scrolldellay="500" style="color:#996600;font-size:19;">SQL - TOP, LIMIT or ROWNUM Clause</marquee>
</td></tr>
<tr><td colspan="2" align=center>
<img src="..\pic\student.jpg" alt=" My LOGO" align="middle" width="147" height="147"/></tr></td>

<tr>
<td style="text-align:left;"><a  href="likeclause.php"><img src="../pic/previous1.gif" width=90 height=30/></a></td>
<td style="text-align:right;"><a  href="orderby.php"><img src="../pic/next1.gif" width=90 height=30/></a></td>
</tr>
<tr><td colspan="2">
<hr>
<p>The SQL <b>TOP</b> clause is used to fetch a TOP N  number or X percent records from a table.</p>
<p><b>Note:</b> All the databases do not support TOP clause. For example MySQL supports <b>LIMIT</b> clause to fetch limited number of records and Oracle uses <b>ROWNUM</b> to fetch limited number of records.</p>
<h2>Syntax:</h2>
<p>The basic syntax of TOP clause with SELECT statement would be as follows:</p>
<pre class="output">SELECT TOP number|percent column_name(s)
FROM table_name
WHERE [condition]</pre>
<h2>Example:</h2>
...
+----+----------+-----+-----------+----------+</pre>
<p>Following is an example on SQL server, which would fetch top 3 records from CUSTOMERS table:</p>
<pre class="output">SQL&gt; SELECT TOP 3 * FROM CUSTOMERS;</pre>
...
|  3 | kaushik |  23 | Kota      | 2000.00 |
+----+---------+-----+-----------+---------+</pre>
<p>If you are using MySQL server, then here is an equivalent example:</p>
...
|  3 | kaushik |  23 | Kota      | 2000.00 |
+----+---------+-----+-----------+---------+</pre>
<p>If you are using Oracle server, then here is an equivalent example:</p>
<pre class="output">SQL&gt; SELECT * FROM CUSTOMERS
WHERE ROWNUM &lt;= 3;</pre>
<p>This would produce the following result:</p>
<pre class="output">+----+---------+-----+-----------+---------+
